...
export const useSlots = (props: UseSlotsProps) => {
  const { eventLength, minimumBookingNotice = 0, date, users, eventTypeId, bookings  } = props;
  console.log(props);
  ...
  useEffect(() => {
    ...
    Promise.all(
      ...
      let loadedSlots: Slot[]|void = results[0];
      if (results.length === 1) {
        loadedSlots = loadedSlots?.sort((a, b) => (a.time.isAfter(b.time) ? 1 : -1));
        setSlots(loadedSlots);
        /*const finalSlots = loadedSlots.filter((slot) => !bookingsArr.includes(slot.time.toString()));
        setSlots(finalSlots);*/
